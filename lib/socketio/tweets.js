...
module.exports = function(app, lib) {
  ...
  app.twitter.get(
    'users/lookup',
    { screen_name: app.configjs.twitter.users.join(',') },
    function(err, data, response) {
      ...
      var searchConf = lib.twitter.config.parse(
        app.configjs.twitter.tracks,
        app.configjs.twitter.users,
        userids
      );
      app.debug('starting streams...');
      var hashStream = app.twitter.stream(
        'statuses/filter',
        searchConf.hashtags
      );
      ...
      var streamParseSendTweet = lib.twitter.parseSendTweet(
        'streamtweet',
        app,
        lib
      );
      stream.on(
        'tweet',
        streamParseSendTweet.bind(null, app.socketio, tweetBuffer)
      );
      ...
      var pastParseSendTweet = lib.twitter.parseSendTweet(
        'pasttweet',
        app,
        lib
      );
      ...
      app.socketio.on('connection', function(socket) {
        ...
        app.twitter.get(
          ...
          function(err, data, response) {
            ...
            data.statuses
              .reverse()
              .forEach(pastParseSendTweet.bind(null, socket, pTweetBuffer));
          }
        );
      });
    }
