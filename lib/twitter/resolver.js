...
var exports = module.exports = function (queue, url) {
  if (/^https?:\/\/twitter.com\/.*\/photo\/\d/.test(url.expanded_url)) return queue.sadd(exports.resolveGif, url)
  if (/^https?:\/\/instagram.com\/p\//.test(url.expanded_url)) return queue.sadd(exports.resolveInsta, url)
...
exports.resolveInsta = function (url, callback) {
  ...
  request(nurl, function (error, response, body) {
    if (!error && response.statusCode === 200) {
      ...
      request(urljoin(nurl, 'media/?size=l'), function (error, response, body) {
        if (error) return callback(error)
        if (response.statusCode !== 200) return callback('invalid response')
        url.expanded_url_img = response.request.href
        callback(null)
      })
    } else callback(error || 'error')
  })
